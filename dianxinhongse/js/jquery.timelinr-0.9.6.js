...
jQuery.fn.timelinr = function(options) {
	...
	$(function() {
		...
		if($(settings.datesDiv).length > 0 && $(settings.issuesDiv).length > 0) {
			...
			if(settings.touchEnabled == "true") { //支持滑动
				var startX, startY;
				var lastChild, firstChild;
				var currentDate = $(settings.datesDiv).find('a.' + settings.datesSelectedClass);
				if(currentDate.parent().is('li:last-child')) {
					lastChild = currentDate.parent();
				}
				if(currentDate.parent().is('li:first-child')) {
					firstChild = currentDate.parent();
				}

				$(settings.containerDiv).on("touchstart", function(ev) {
					startX = ev.originalEvent.touches[0].pageX;
					startY = ev.originalEvent.touches[0].pageY;
				});
				$(settings.containerDiv).on("touchmove", function(ev) {
					//解决android不能触发touchend
					ev.preventDefault();
				});
				$(settings.containerDiv).on("touchend", function(ev) {
					var endX, endY;
					...
					switch(direction) {
						...
						case 3:
							...
							var currentDate = $(settings.datesDiv).find('a.' + settings.datesSelectedClass);
							if(currentDate.parent().is('li:last-child')) {
							if(!lastChild) lastChild = currentDate.parent();
								//
								if(lastChild && lastChild.is('li:last-child')) {
						...
						case 4:
							...
							//这里处理不太完善，，，先默认当前就是第一个，，，
							if(currentDate.parent().is('li:first-child')) {
							if(!firstChild) firstChild = currentDate.parent();
								//
								if(firstChild && firstChild.is('li:first-child')) {
